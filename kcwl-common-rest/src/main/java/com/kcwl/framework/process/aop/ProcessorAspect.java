...
public class ProcessorAspect {
    ...
    public Object around(ProceedingJoinPoint joinPoint) throws Throwable {
        ...
        //构建校验参数
        Class<? extends KcProcessInterface>[] pres = handlers.before();
        doPreHandler(args[0], pres);

        Object returnArgs = joinPoint.proceed();
        Class<? extends KcProcessInterface>[] posts = handlers.after();
     ...
     * @date: 2023/2/1 18:10
     */
    private void doPreHandler(Object args, Class<? extends KcProcessInterface>[] pres) {
        for (Class<? extends KcProcessInterface> clazz : pres) {
     ...
     * @date: 2023/2/1 18:10
     */
    private void doPostHandler(Object args, Class<? extends KcProcessInterface>[] posts, Object returnArgs, Class<?> returnType) {
        for (Class<? extends KcProcessInterface> clazz : posts) {
            Class paramType = ResolvableType.forType(clazz.getGenericInterfaces()[0]).getGeneric(0).resolve();
     ...
     * @date: 2023/2/1 18:10
     */
    private void doHandler(Object args, Class<? extends KcProcessInterface> clazz) {
        Class resolve = ResolvableType.forType(clazz.getGenericInterfaces()[0]).as(KcProcessInterface.class).getGeneric(0).resolve();
        Object params = getParams(args, resolve);
        KcProcessInterface kcProcessInterface = SpringUtil.getBean(clazz);
        kcProcessInterface.process(params);
    }
